...
var fftCtx4 = wavefft4.getContext("2d");
...
waveCtx.clear(); zoomCtx.clear(); fftCtx.clear(); fftCtx4.clear()
...
function zoomUpdate() {
  ...
  drawWavespec4(fftCtx4, fftchunk);
...
function drawWavespec(cx, d) {
  ...
  if (guidetoggle.checked) {
    ...
    for (var i = 1; i < 96; i++) {
      ...
      cx.moveTo(x * w, h / 2 - h * 0.03 * (5 + 2 * (i == 1) + 5 * (i % 4 == 0)));
      cx.lineTo(x * w, h / 2 + h * 0.03 * (5 + 2 * (i == 1) + 5 * (i % 4 == 0)));
      var nam = "C";
      switch (i % 12) {
        case 1:
          nam = "C#";
      ...
      cx.labelText(nam + Math.floor(i / 12), x * w, 20, "red");
    }
    cx.strokeStyle = "#f00";
    cx.stroke();
    cx.closePath();
...
function drawWavespec4(cx, d) {
  var df = sampleRate / (2 ** (fftexp.value - 2));
  var fftmax = freqmax.value / df;
  var fftmin = freqmin.value / df;
  var plotted = d.slice(fftmin, fftmax);
  var h = cx.canvas.height;
  ...
  if (guidetoggle.checked) {
    ...
    for (var i = 1; i < 80; i++) {
      freq = 440 * (2 ** ((i - 57) / 12));
      x = (freq - freqmin.value) / (freqmax.value - freqmin.value) * 4;
